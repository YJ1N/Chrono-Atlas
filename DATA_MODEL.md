...
**`Lane` 은 화면에 그려지지 않지만 죽은 필드가 아니다.** Phase 2R 에서 Y축이 연속 중요도로 바뀌며 렌더링 역할을 잃었고, Phase 4 에서 제거하려다 살아 있는 역할을 발견했다 — `engine/index/lod.ts` 의 점유 격자가 `laneId` 를 키로 쓴다. 같은 x-빈이라도 레인이 다르면 둘 다 살아남으므로, 레인은 **같은 시각의 서로 다른 종류를 함께 보이게 하는 LOD 분산 축**이다. 제거하면 같은 해의 전투와 과학 발견 중 하나만 남는다 (ADR-018).

이름("트랙")과 실제 역할("분산 축")이 어긋나 있는 것이 남은 부채다.
